...
class cloudHandler
{
	...
	cloudHandler()
	{
		...
		pcl_pub8=nh.advertise<sensor_msgs::PointCloud2>("pcl_filtered",1);
		pcl_pub9=nh.advertise<sensor_msgs::PointCloud2>("pcl_cluster",1);
		pcl_pub10=nh.advertise<sensor_msgs::PointCloud2>("pcl_convexHull",1);
	...
	void cloudCB(const sensor_msgs::PointCloud2 &input)
	{
		...
		//get the figures in mini-clusters
		pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);
		tree->setInputCloud(cloud_projected.makeShared());
		...
		ec.setInputCloud(cloud_projected.makeShared());
		ec.extract(cluster_indices);
		
		int j=0;
		pcl::PointCloud<pcl::PointXYZ> clusters[cluster_indices.size()];
		...
		for(std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin();it!=cluster_indices.end();++it)
		{
			...
			pcl::toROSMsg(*cloud_cluster,output9);
			output9.header.frame_id="camera_link";
			pcl_pub9.publish(output9);
			
			pcl::copyPointCloud(*cloud_cluster,clusters[j]);
						
			j++;
		}
		
		//convex hull
		pcl::PointCloud<pcl::PointXYZ> cloud_hull[cluster_indices.size()];
		pcl::ConvexHull<pcl::PointXYZ> chull;
		sensor_msgs::PointCloud2 output8;
		sensor_msgs::PointCloud2 output10;
		switch (cluster_indices.size())
		{
			case 0:
				std::cout<<"no hay nada"<<std::endl;
				break;
			case 1:
				//std::cout<<"solo hay 1 nube de "<<clusters[0].points.size()<<" puntos"<<std::endl;
				chull.setInputCloud(clusters[0].makeShared());
				chull.reconstruct(cloud_hull[0]);
				pcl::toROSMsg(cloud_hull[0],output10);
				output10.header.frame_id="camera_link";
				pcl_pub10.publish(output10);
				std::cout<<"solo hay 1 nube de area "<<pcl::calculatePolygonArea(cloud_hull[0])<<" y radio "<<sqrt(pcl::calculatePolygonArea(cloud_hull[0])/M_PI)<<std::endl;
				pcl::toROSMsg(clusters[0],output8);
				output8.header.frame_id="camera_link";
				pcl_pub8.publish(output8);
				break;
			default:
				std::cout<<cluster_indices.size()<<" metodo de la burbuja"<<std::endl;
				pcl::PointCloud<pcl::PointXYZ> aux;
				for(int i=0;i<((cluster_indices.size())-2);i++)
				{
					for(int k=1;k<((cluster_indices.size())-1);k++)
					{
						if(clusters[i].points.size()<clusters[k].points.size())
						{
							pcl::copyPointCloud(clusters[k],aux);
							pcl::copyPointCloud(clusters[k+1],clusters[k]);
							pcl::copyPointCloud(aux,clusters[k+1]);
						}
					}
				}
				for(int i=0;i<=cluster_indices.size()-1;i++)
				{
					chull.setInputCloud(clusters[i].makeShared());
					chull.reconstruct(cloud_hull[i]);
				}
				for(int i=0;i<=cluster_indices.size()-1;i++)
				{
					if(sqrt(pcl::calculatePolygonArea(cloud_hull[i])/M_PI)<0.17 && sqrt(pcl::calculatePolygonArea(cloud_hull[i])/M_PI)>0.16)
					{
						pcl::toROSMsg(cloud_hull[i],output10);
						output10.header.frame_id="camera_link";
						pcl_pub10.publish(output10);
						std::cout<<"nube "<<i<<" de area "<<pcl::calculatePolygonArea(cloud_hull[i])<<" y radio "<<sqrt(pcl::calculatePolygonArea(cloud_hull[i])/M_PI)<<std::endl;
						pcl::toROSMsg(clusters[i],output8);
						output8.header.frame_id="camera_link";
						pcl_pub8.publish(output8);
					}
				}
	...
	protected:
		...
		ros::Publisher pcl_pub, ind_pub, coef_pub, ind_pub2, coef_pub2, pcl_pub2, pcl_pub3, pcl_pub4, pcl_pub5, pcl_pub6, pcl_pub7, pcl_pub8, pcl_pub9, pcl_pub10;
};
